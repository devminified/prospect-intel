...
- **M88–M91** — Combined-manager invites + multi-profile picks. New `team_invites.upwork_assignments_json` jsonb (M88) carries `[{profile_id, role}]`. Service layer translates seven invite presets — outbound `manager`/`lead_gen`/`cold_caller`/`closer`, `upwork_bidder` + `upwork_manager` (one or more profiles), and `combined_manager` (outbound manager + auto-snapshot of every active profile that has no manager) — into `team_role` + assignments at create time, then fans out to `upwork_profile_members` on redeem (M89). `/settings/team` UI shows a grouped Outbound / Upwork / Combined dropdown plus a checkbox grid for picking multiple profiles, disabling already-managed profiles for the manager preset (M90 single→M91 multi). Snapshot at create time — profiles added later are NOT auto-included.
