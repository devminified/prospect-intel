...
- **M88–M90** — Combined-manager invites. New `team_invites.upwork_assignments_json` jsonb (M88) carries `[{profile_id, role}]`. Service layer translates seven invite presets — outbound `manager`/`lead_gen`/`cold_caller`/`closer`, `upwork_bidder` + `upwork_manager` (single profile), and `combined_manager` (outbound manager + auto-snapshot of every active profile that has no manager) — into `team_role` + assignments at create time, then fans out to `upwork_profile_members` on redeem (M89). `/settings/team` UI shows a grouped Outbound / Upwork / Combined dropdown plus a conditional profile picker that disables profiles already managed (M90). Snapshot at create time — profiles added later are NOT auto-included.
