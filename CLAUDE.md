...
- **Phase 4C (M25–M26)** — Reply-outcome feedback into planner + daily 08:00 UTC auto-gen cron. Post-ship hygiene rounds added: duplicate detection, optional ICP hard filters (LinkedIn/Instagram/Facebook/phone), planner now sees hard filters at plan time, self-open suppression, **M28 — business email discovery + `require_reachable` ICP filter**, and **M29 — pre-batch ICP enforcement** (rating/review_count/business_status hard-filtered at import; `batches.count_filtered_below_icp` + `count_duplicates_skipped` persisted). `archive/phase-4c-learning.md`
